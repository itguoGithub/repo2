...
public class OrderServiceImpl implements OrderService {
	...
	public void add(TbOrder order) {
		...
		/*
		 * //1.从redis中提取购物车列表 List<Cart> cartList= (List<Cart>)
		 * redisTemplate.boundHashOps("cartList").get(order.getUserId());
		 * 
		 * 
		 * //2.循环购物车列表添加订单 for(Cart cart:cartList){ TbOrder tbOrder=new TbOrder(); long
		 * orderId = idWorker.nextId(); //获取ID tbOrder.setOrderId(orderId);
		 * tbOrder.setPaymentType(order.getPaymentType());//支付类型
		 * tbOrder.setStatus("1");//未付款 tbOrder.setCreateTime(new Date());//下单时间
		 * tbOrder.setUpdateTime(new Date());//更新时间
		 * tbOrder.setUserId(order.getUserId());//当前用户
		 * tbOrder.setReceiverAreaName(order.getReceiverAreaName());//收货人地址
		 * tbOrder.setReceiverMobile(order.getReceiverMobile());//收货人电话
		 ...
		 * tbOrder.setSellerId(order.getSellerId());//商家ID
		 * 
		 * double money=0;//合计数 //循环购物车中每条明细记录 for(TbOrderItem
		 * orderItem:cart.getOrderItemList() ){ orderItem.setId(idWorker.nextId());//主键
		 * orderItem.setOrderId(orderId);//订单编号
		 * orderItem.setSellerId(cart.getSellerId());//商家ID
		 * orderItemMapper.insert(orderItem);
		 * money+=orderItem.getTotalFee().doubleValue(); }
